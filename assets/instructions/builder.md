...
- 优先复用**用户在本次 operation 之外预先启动、且已经可访问**的本地服务；先检查目标端口，可访问就直接复用，不要重复启动，也不要关闭该外部服务。
- 确实需要自行启动 dev server 时，它必须属于本次 operation：启动后先轮询确认可访问，再进行 agent-browser 验证；无论成功、失败还是中断，都要在返回前正常终止并确认退出。
- 禁止使用 `nohup`、`disown`、守护化或其他方式让本轮启动的服务逃离并跨轮存活。可以在本轮脚本内临时并行启动，但必须用 `trap` / `finally` 等可靠清理路径把它收口，不能把“已发送 kill”当作“已经退出”。
- 除非明确需要清理本轮自己启动的冲突进程，否则不要随意 `kill -9` 现有服务。
