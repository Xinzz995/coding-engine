...
### 候选 Dogfood 回归：长提示词受管传输

0.34.0 候选在 Go 首次真实运行中于 Builder 启动前失败：正式 `builder.md` 渲染后超过
supervisor 单字段 4096 字符边界，Validator 也超过同一边界。该边界适用于协议身份、路径和环境
字段，不应迫使合法模型提示词进入命令参数，也不能通过提高所有安全字段上限规避。

修复裁决：复用 Final Review 已验证的固定代理和密封临时目录。Codex、Claude 从标准输入取得完整
提示词；Cursor 继续使用明确的 16 KiB 参数上限，超限时拒绝且不截断。supervisor 只接收固定代理
路径和配置路径，Runner 仍由原生 CLI 执行，权限、进程树、超时、delegated delta 与恢复合同不变。

| 黄金原则         | 适用性与设计裁决                                                                    | 完成证据                                                                   |
| ---------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| 可证伪完成合同   | 随包 Builder、Validator 必须在三种 Runner 中逐字节到达；supervisor 参数不得携带原文 | 真实指令资产测试分别覆盖无模型和固定模型；缺字节、截断或进入错误通道即失败 |
| 生成方不自签通过 | Runner 自述不参与此修复裁决，由受管测试进程捕获实际输入并机械比对                   | 捕获结果与原始文件逐字节相等，现有 gate/Validator 凭证规则不变             |
| 自治与防线同步   | 不扩大 Agent 权限；临时提示词文件只读、目录树固定、受管进程结算后才清理             | 临时目录身份、篡改、清理、超时与后代进程回归继续通过                       |
| 复用原生执行能力 | 不自建模型客户端；Codex/Claude 使用原生 stdin，Cursor 保留原生参数方式              | 三种 Runner 参数与模型选择测试通过，核心 workspace 协议不加入供应商字段    |
| 真实失败固化回归 | 以大于 4096 字符的正式 Builder/Validator 作为失败样本                               | 定向测试、全量测试、三系统 CI 和同一新候选的三仓 Dogfood 全部通过          |

修复只有在新 candidate run 和摘要生成、三个项目从全新 workspace 重跑后才算完成；旧候选
`30769461753` 永久失效，不得进入 npm staging。
