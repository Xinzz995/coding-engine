...
**Goal:** 让每次真实 Builder/Validator 进程调用都留下可恢复诊断的机械凭证：实际结局、退出码、耗时和有界输出尾部；终端仍实时显示原生 runner 输出，异常原因不再随进程结束丢失。
...
1. `runAgent` 对 completed/error/timeout 都返回非负 `durationMs`、`exitCode` 和最多 2000 字符的 stdout/stderr 合并尾部；stdout/stderr 继续实时转发，正常路径等 pipe 关闭后结算，超时仍等待整棵进程树退出后才结算。
2. 每个实际启动的 Builder/Validator 都在本轮 iteration 记录 invocation：耗时与退出码始终存在；只有异常/超时才持久化诊断尾部，成功输出不落 evidence，避免噪声与不必要的数据留存。
3. evidence 读取端严格守卫新增字段，旧记录继续可读；status JSON/文本与 report 能显示最近调用耗时，并在异常时显示退出码和转义后的诊断尾部。
