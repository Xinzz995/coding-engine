...
清理只在受管进程正常结束、没有超时或残留后代、身份与目录树仍一致后进行；其余情况保留现场并返回 `unverifiable`。POSIX 只有在仍能通过创建时绑定的句柄证明对象、属主和内容，并机械收紧 POSIX 权限位后，才记录 `restricted`；其他保留场景明确记录保护状态不可验证。
...
- 临时域创建时按平台能力请求最小权限；POSIX 保留现场只有经已绑定句柄收紧、回读和返回路径
  复核后，才能证明收口时根目录及固定树的组/其他用户权限已移除。
...
- `restricted` 不证明管理员、既有打开句柄或收口前的读取与复制行为，也不证明 Review 内容有效；
- `restricted` 不证明 POSIX 扩展 ACL 已被移除，字段中的机制与范围必须始终随结果保留；
- Windows v1 不把 Node 的 mode 数字当作 ACL 证明，所有保留现场都明确标记保护状态不可验证；
...
|---|---|---|---|
| AC-1 | 五类临时域创建后立即冻结系统临时父目录、直接子目录关系、目录类型和平台可用的稳定身份；后续使用与清理都重核同一对象 | 根目录、祖先、路径别名或 Windows reparse 发生变化时拒绝继续 | 每类临时域的身份替换测试；Windows 普通用户真实证明固定文件句柄存续时根目录与父目录无法被重命名，并证明空固定树或安全动态树被 junction 替换后清理会保留现场且不触碰外部目标 |
...
| AC-4 | 清理前重新证明创建身份和固定拓扑；身份不一致时不通过路径执行 `chmod`、rename 或递归删除。POSIX 只允许用仍绑定原对象的句柄收紧权限，并把保护证明或不可验证原因写入结构化保留结果；Windows 不声称具备 ACL 证明。只有仍能绑定冻结目录时才报告真实保留路径；对象位置无法证明时返回独立的不可验证状态与有界候选路径 | 替换目录和外部 canary 保持原字节；权限漂移但身份、内容和进程终点仍可证明时，绑定句柄收紧为目录 `0500`、固定文件 `0400`；删除已完成后抛错或对象移走时不得虚构保留路径；其余明确要求人工隔离；诊断只含已知路径、阶段和引擎生成原因 | 权限漂移、未结算进程、清理前替换、删除后失败、外部 canary、Windows 原生、创建回滚与诊断脱敏测试 |
...
| AC-6 | 初始化中途失败也只清理身份仍匹配且尚未交给受管进程的临时域；其余保留并明确保护状态不可验证 | 写入第二个文件失败、权限设置失败或进程启动边界中断时不存在越权删除，也不把无结构化证明的现场称为已保护 | 公共创建阶段与审查包、调用、版本、探测、status 各调用点的初始化注入失败测试 |
...
4. 清理是有结果的收口步骤，不放在会掩盖原错误的无条件 `finally` 中。调用者必须把“已删除”与
   “保留现场”纳入该轴裁决。保留结果还必须区分“POSIX 已绑定句柄完成收紧”和“保护状态不可验证”；
   前者只陈述收口时返回路径的访问位，不恢复 Review 有效性，也不扩大成历史保密承诺。位置无法
   绑定原对象时必须作为第三种收口状态失败关闭，不能把候选路径写成已保留事实。
...
| 3. 自治扩大必须增加防线 | 本轮不扩大模型权限；它收紧现有自动读取与删除。删除只有在身份和进程终点均确定时发生，不确定即保留，且不增加自动垃圾回收。权限收紧只允许使用仍绑定原对象的句柄；身份异常时保持零路径 chmod。 | 外部 canary、身份异常零 chmod/rm、权限漂移绑定句柄收紧、保留现场保护状态 |
...
2026-08-02 已确认保留现场保护边界：能机械证明才记录 `restricted`；否则记录保护状态不可验证并
继续阻断。Windows v1 接受 `platform-unsupported`，不以 POSIX mode 冒充 ACL 证明。若后续要求
Windows 也给出等价证明，必须新增经过原生攻击测试的 ACL 适配器并另立架构裁决，不能静默升级承诺。
