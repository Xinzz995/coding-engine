...
`0d5deeb2217f798dd1337197ebbb23fd18a76d0a` 必须保持不变。

## RC6 十项共同身份

以下十项必须同时匹配；缺失或任一项不同都立即停止，不得把结果记为 RC6 证据：
...
回执、旧 Final Review、旧报告和旧 PR 结论都不能作为 RC6 证据；相同版本字符串、RC5 与 RC6
相同的 tarball 字节、相同来源仓库或曾经通过的检查也不能恢复这些证据。
...
| 托管环境            | 同一 artifact 下载与同一摘要核对                                                              | 仓库外全新安装 | npm 真实命令入口检查                                 | 已记录结论   |
| ------------------- | --------------------------------------------------------------------------------------------- | -------------- | ---------------------------------------------------- | ------------ |
...
表中“成功”只证明同一个 RC6 候选包能在三个参考系统安装和启动。candidate workflow、相同 tarball
字节、相同来源提交或旧 PR 检查都不能替代下面的三仓 Shadow Dogfood、各仓原生门禁和人工证据对账。
...
1. **seed 阶段**：先在未改写的 seed 提交
   `0d5deeb2217f798dd1337197ebbb23fd18a76d0a` 上记录本地既有检查，并等待该 exact HEAD 的 Policy
