---
title: coding-x 0.38.1 coding-engine 候选证明 R3
status: candidate-only
updated: 2026-08-23
scope: root
---

# coding-x 0.38.1 coding-engine 候选证明 R3

本文是 coding-engine 自托管路径的自包含待执行候选说明，不是已经完成的候选证明或正式发布证明。
PR #334 已在候选调用前开放，但这只满足运行前提，不表示种子、当前 head 的 Story 凭证、Shadow
Final Review、远端必需检查或 owner 机器证明已经完成。下列步骤必须按顺序执行并分别取得证据。
...
5. 在 doctor 和 apply-prd 之前，先用同一绝对候选 CLI 初始化本次 R3 运行专用、此前从未创建或复用的
   全新 workspace：

   ```bash
   <candidate-cli> workspace init --workspace <new-r3-workspace>
   ```
...
6. 只在步骤 5 创建的同一个 workspace 中运行 Shadow doctor：
   ...
   <candidate-cli> doctor --shadow --candidate-evidence <packed.json> \
     --json --workspace <new-r3-workspace>
   ```
...
8. 再次确认 PR #334 仍开放且指向当前分支后，只允许执行一次 coding-x codex Shadow 调用：

   ```bash
   <candidate-cli> codex --shadow --candidate-evidence <packed.json> \
     --workspace <new-r3-workspace> --no-open
   ```

   Builder 产生本地提交后，外层流程只负责及时把该原样提交推送到 PR #334 的当前分支，不修改提交
...
PR #334 在 staging 读取最终机器证明前必须保持开放且永不合并。coding-x 0.38.1 发布收口完成后才
关闭该 PR，并保留其中的最终机器证明作为历史证据。这份待执行说明本身不能供 staging 读取，也不
授权 npm staging、2FA 批准、移动 `next` 或 `latest`、创建标签或正式发布。

PR #332 与 PR #333 是相互独立且与 R3 分离的失败现场；它们的提交、workspace、Story 凭证、Review
和评论都不得在 R3 复用。R3 必须在上述全新 workspace 中建立并绑定自己的结果，且在结果真正产生前
不得预先声称种子、Story 凭证、Final Review、远端检查或 owner 证明已经完成。
