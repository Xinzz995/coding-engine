...
日常 PR Quality Gate 会按质量契约 `paths` 只运行当前净变化适用的检查；未知变化自动全量。受管
Ruleset 无绕过并要求 PR 使用最新默认分支，合并后不重复项目检查；每周与人工触发始终执行完整矩阵，
CodeQL 仍在 PR 与默认分支运行。这个调度优化不改变候选发布边界：候选构建仍要完成自身打包、三系统
