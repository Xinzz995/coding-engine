...
全部检查通过，工作树干净，候选提交就是当前远端 `main`。首次 0.30.0 使用一次性的“机械
CI + owner 人工 Bootstrap”：PR 最新提交必须通过全部仓库检查，但不声称完成正式本地 AI
Review，也不得用候选版本为自己签发正式结果。0.30.0 发布并由独立 Policy PR 固定后，后续
版本恢复稳定版评估候选版的常规流程。

### 2. 暂存固定候选

从 GitHub Actions 手动运行 `Stage npm candidate`，分支必须选 `main`，输入精确稳定版本，例如
`0.30.0`。工作流会先在无发布身份的任务中执行完整检查、构建并保存候选包；只有该任务成功，
独立暂存任务才取得短期 OIDC 身份。
