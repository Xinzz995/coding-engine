...
| 20 | 真实 Runner 的 provider/认证/网络异常必须按发生阶段区分：Developer 异常沿用未完成/重试语义；Validator 异常保留候选、退出 5 且无凭证；coordinator 无法证明子进程集合清空时一律隔离并退出 2。iteration/status/report 留下结局、退出码、收口耗时与有界诊断，成功调用不持久化 transcript | 2026-07-22 Claude/Codex 双 runner dogfood / ADR-016/021/023 | fake 402 分别注入 Developer 与 Validator；对账候选、退出码、lease 结算、`builderInvocation`/`validatorInvocation`、status 与 report；另注入 containment 证明丢失并确认退出 2 |
