...
| 30 | 远端 provider 单次输入超限时 source/diff 均不截断，首次请求前按完整 prompt 拆成不超过八个无损片段并覆盖完整评审空间；每片都有效才合并，重复 finding 保留最高严重度，任一片失败或继续超限都阻断 | ADR-018 / 0.30.7 | prompt 预算与 source/diff 双向分片测试；模拟 provider 拒绝后核对逐片调用与聚合；真实含多份来源的 PR 核对 Check Run 和调用回执 |
| 31 | 多个 PR 同时触发 AI 时，全仓模型 job 进入保留等待项的串行队列，单轴分片保持顺序调用；新 head 取消自己的旧运行但不让其他 PR 的待审任务静默消失 | ADR-018 / 0.30.7 | workflow 合同测试；同时触发人工与 Dependabot PR，核对 job 排队、最新 SHA Check Run 和无请求风暴 |
...
| 35 | Copilot 远端评审只加载默认分支可信指令；PR 内容无法启用工具、读取环境秘密或覆盖评审角色；CLI 版本、每次调用的实际模型和真实调用用量进入当前 head 回执，跨分片 auto 路由可以不同但必须完整留证，缺失用量不得伪造为零；同一调用允许同模型的多条无工具回复与说明文字，但必须且只能提取一个 JSON 对象；版本漂移、单次路由身份冲突、多个 JSON、工具请求、五分钟超时或进程异常均为 unverifiable | ADR-019 / 0.31.2 | 固定版本 adapter 单元测试；恶意 prompt 真实探针；coding-engine 与非 Node 下游正常 PR 的三轴 Check Run |
