...
| 30 | GitHub Models 单次输入超限时完整 diff 不被截断，source 无损拆成不超过八片；每片都有效才合并，重复 finding 保留最高严重度，任一片失败或继续超限都阻断 | ADR-018 / 0.30.4 | mock 413 后核对逐片调用与聚合；真实含多份来源的 PR 核对 Check Run 和模型调用日志 |
