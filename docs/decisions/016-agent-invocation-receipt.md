...
# 016. 普通轮次为已权威结算的真实 Agent 子进程调用留下有界调用凭证
...
> 调用凭证随普通 iteration 成批持久化，不是逐调用日志；同轮后续发生 proof-missing 时，
> 此前已结算的调用也不会单独落盘。
...
iteration 增加可选 `builderInvocation` / `validatorInvocation`。`recordIteration` 只在普通轮次写入点附加当时已经权威结算的两侧调用：记录 duration 与 exitCode，只有 error/timeout 保存 `diagnosticTail`，completed transcript 不持久化。调用凭证不是逐调用即时追加；若同轮后续 Validator 发生 proof-missing，循环会在 `recordIteration` 前永久隔离，整轮不写普通 iteration 或 invocation receipt，此前已结算的 Builder 也不会单独进入 evidence/status/report。proof-missing 只保留安全协议、隔离状态和受保护现场。旧 evidence 缺字段继续可读。
