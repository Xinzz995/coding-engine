...
分片引用均为输出无效，不进入失败裁决。
0.30.9 在外部 Python PR 上正确挡住真实问题并在修复后正常通过，但 coding-engine 结构治理
PR 连续三次收到“测试覆盖充分、无需修改”的 medium finding。更高限流档模型真实调用在两
分钟内持续 429，其他低限流档要么不支持严格 schema，要么产生更空泛的高严重度误报，因此
不把切换模型当作恢复方案。adapter 改为接受去掉补丁控制前缀后仍能在同文件逐字定位的代码
证据，并机械拒绝明确表示无需修改的伪 finding；第一次无效时仅对同一分片追加拒绝原因重审
