...
Builder 返回并完成对账后固定本轮唯一验收 HEAD。项目机械检查与 TDD 门禁各自在启动前、结束后
核对，Validator 请求建立前和返回后再次核对；任一边界观察到变化或不可读时立即停止，不接受该阶段
结果、不签发凭证、不增加 retry，并在新 HEAD 可读时先撤销其他 Story 的旧提交凭证。validation-only
保留既有候选；普通实现轮从 ADR-023 起在环境、协议或 HEAD 不可验证时同样保留未验收候选。下一次运行重新走完整链路，禁止把不同 HEAD 的结果
拼接成一次通过。工作树瞬时变化与 H1→H2→H1 仍属于 Issue #91 的隔离检出边界。

下述恢复分类最初只用于“凭证过期但已有 `passes=true` 候选”的 validation-only 轮；ADR-023 将其中
Validator 的 `unverifiable` 分支扩展到普通实现候选。机械检查与 TDD 对普通实现轮的既有裁决不变：

| 结果           | 判定                                                                                                         | 状态转移                                                                                            |
| -------------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `passed`       | 适用机械/TDD 检查全部通过；或 Validator 返回合法 passed claim                                                | 保持候选并继续下一验证步骤；Validator passed 后签发凭证                                             |
| `failed`       | 项目命令正常启动、正常结束并按冻结合同返回非零；确定的 TDD 政策违规；合法 Validator failed                   | 清除候选与凭证，沿用既有 retry、升级与 blocked 规则；本次运行停止，下次启动重新预检后，未 blocked 时才进入 Builder |
...
实现已知有错，环境或协议不可验证时不能因此重复调用 Builder。ADR-023 进一步把相同规则扩展到
普通 Builder 刚产生的新候选；明确失败的 retry、升级与 blocked 语义不变。validation-only 启动预检不解析不会调用的 Builder 路由，因此明确失败后不能在同一次
运行中直接转入 Builder；下一次正式启动会先按新状态完成全部 Builder 路由预检。
