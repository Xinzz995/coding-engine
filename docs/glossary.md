...
**质量契约**
受 Git 管理的 `.coding-x/quality.json`：声明项目原生检查、Spec 与工程标准来源、风险触发条件、默认分支、发布引用和远端所需检查。它是政策输入，不是通过结果；缺失或非法时不得静默降级。
禁用：质量配置、CI 配置

**三轴评审**
彼此隔离的 Spec 评审、工程标准评审和风险触发的深度结构评审。三者分别保留问题与结论，不由总评审者重新排序、合并或淡化。
禁用：三层评审（“层”容易误解为同一评审逐层加深）

**质量三态**
质量检查唯一允许的结论：passed 表示证据完整且绑定当前提交；failed 表示发现明确问题；unverifiable 表示缺少资料、凭据、有效结果或远端能力。failed 与 unverifiable 都阻断。
禁用：未知通过、降级通过

**本地质量反馈**
`quality review` 或 `/review-loop` 针对当前提交生成的 workspace receipt 与阅读视图，用于在 PR 前提早发现问题。它可被同权限进程改写，不是共享交付记录，也不能作为 GitHub 通过凭证。
禁用：人审包、审查证明

**交付就绪**
GitHub PR 最新提交的项目检查与所有适用评审均通过，实际远端规则健康，且没有过期或不完整异常。story 全部有效通过只代表实现已验证，不自动等于交付就绪。
禁用：开发完成、全部 story 通过

**异常交付**
管理员因紧急情况绕过或临时改变远端规则后形成的交付状态。它必须在受 Git 管理的异常文件中绑定精确提交、责任人、原因、期限、审计链接和后续事项；关闭前始终为 unverifiable，不能改名为通过。
禁用：紧急通过、临时放行

**收口**
一轮循环或功能分支完成后的收尾动作：本地质量反馈 + GitHub PR 门禁与合并 + /compound-docs 沉淀经验、清理活知识熵并收尾任务文档；物理归档须另有显式授权。
禁用：复盘、总结
...
**验证报告**
一次运行的验证证据静态存档（`<workspace>/report.html`）：story 状态与验收标准、门禁配置、截图工件、本地质量反馈、篡改红旗区汇总为零依赖单页。循环结束从引擎冻结的 PRD 快照自动生成，`coding-x report` 则从磁盘随时重生成；state 损坏时只生成“全部未验证”的红色诊断报告且手动命令退出 1。它是本地存档，不证明 GitHub 交付就绪——正常生成退出 0，循环成败的 CI 语义归 status，远端交付以 PR Check 为准。
...
- 收口包含 `/review-loop` 的本地质量反馈、GitHub PR 最新提交的远端门禁与 /compound-docs（沉淀、熵 GC、状态收尾及显式授权后的物理归档）
- 物理归档把完成态阶段文档从 active 区移入历史冷档案；状态收尾只改 status，不自动构成移动授权
- 验证报告可以收录 workspace 中的本地质量反馈，但必须同时标明它不是共享交付凭证；远端结论只以 GitHub Check 与 PR 历史为准
...
- 质量契约定义项目政策；本地质量反馈提供提前发现；GitHub 对最新提交独立重跑项目检查与三轴评审；ruleset 决定是否允许合并
- Spec 与工程标准评审始终独立；深度结构评审只在风险触发时适用；任一适用轴为 failed 或 unverifiable 都不具备交付就绪
- 普通 finding 只有契约允许的严重度可延期；异常交付不豁免 doctor 的漂移结论，修复规则和关闭后续事项后才恢复正常交付状态
