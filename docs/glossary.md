...
**Agent 调用凭证**
引擎对一次已经权威结算的真实 Builder/Validator 子进程调用的机械观察：取得与调用类型相符的收口证明前的墙钟耗时、退出码，以及仅在异常时保留的有界 stdout/stderr 尾部。普通项目代码主动逃逸平台 containment 的行为不在此证明内；受支持 POSIX AI Runner 已启动后被外部终止、以 signal 结束或观察到残留进程时不签发这种完成凭证，而是永久隔离 workspace。输出内容是恢复诊断，不是 provider 事实、账单证明或验收结论。
禁用：调用日志、执行证明、成本凭证
...
**stall 熔断**
空转轮、已经权威结算的 Developer 异常轮或旧兼容路径触发待复核回写的轮次，连续累计达 `--stall-limit`（缺省 3）即提前终止循环（退出码 1）；POSIX 不透明 Runner 外部终止后的永久隔离和结构化 Validator 不可验证都立即停止，不进入 stall 重试。
禁用：空转保护、无进展终止
...
- 每次已经权威结算的 Builder/Validator 都产生 Agent 调用凭证；proof-missing 不伪造普通 iteration，status/report 只显示安全协议与隔离事实
