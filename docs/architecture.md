...
可信收口（ADR-014）进一步收紧这条数据流：loop 的自动报告消费终轮 guard 返回的冻结 PRD 快照并标记来源，不在锁内重新相信磁盘；手动 report 才读取当前磁盘 PRD。state 文件缺失可走 legacy 迁移，文件存在但损坏则所有 story 按未验证渲染，诊断报告原子落盘且手动命令退出 1。`prd-to-json` 在任何 workspace 变更前与首次真实写入前各消费一次 doctor 的工作区锁结论；活锁或无法判定时零写入，不删除 `engine.lock`。双次检查只缩小 TOCTOU 窗口，不等价于持锁。
