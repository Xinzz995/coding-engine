...
| 执行状态 | `src/engine/state.ts` | state.json 读写与迁移、选 story、完成判定、合并视图；`escalated` 路由状态由引擎独占 |
...
| 知识库体检 | `src/doctor/doctor.ts` | `coding-x doctor` 检查 frontmatter / updated / AGENTS.md 索引 / 相对链接，并核对机械门禁与全局模型目录/PRD 映射；runDoctor/renderDoctorReport 纯函数，cli 渲染并定退出码 |
| 状态速览 | `src/status/status.ts` | `coding-x status` 展示 story 状态/难度/升级态、配置模型路由与 evidence 中最近实际命中；`--json` 输出同等机器可读字段，退出码 0/1/2 可作 CI 门禁 |
...
| 仪表盘 | `src/dashboard/server.ts` | HTTP 服务（:7331）+ 自动开浏览器；分开展示完整配置路由与当前阶段实际命中；`coding-x dashboard` 可离线复用 |
...
模型路由有两层真相源：用户级 `~/.config/coding-x/config.json`（可由 `CODING_X_CONFIG` 覆盖）声明各 runner 允许选择的模型 ID；项目 `prd.json.models` 保存当前 runner 与五项路由映射。`prd-to-json` 在 story 定稿后只读全局目录，让用户从中选择五个模型，并按仓库事实派生每个 story 的 `difficulty/difficultyReason` → 启动时 preflight 严格校验 schema、runner，以及本次可能实际调用的 ID 是否在目录中 → loop 按 `difficulty + state.escalated + CLI override` 解析本轮实际模型 → agent 返回后引擎恢复 agent 对 `escalated` 的任何改写，并把实际模型、来源、档位和升级触发写入 `evidence.jsonl` → dashboard/status/report 分开展示「配置路由」与「实际命中」，不用前者伪装后者。
