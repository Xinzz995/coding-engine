...
| 进程树终止 | `src/engine/process-tree.ts` | agent 与机械门禁共享的跨平台超时收口：POSIX 进程组 SIGTERM→SIGKILL 并探活；Windows 等待 `taskkill /T /F` 返回。当前只覆盖调用方仍存活的 timeout 路径，且 Windows 返回不能证明持续进程集合已清空；completed/error、父进程崩溃、信号收口和终止失败后的隔离尚待 Issue #106 / ADR-021 实现 |
...
| 机械门禁与回写 | `src/engine/gate.ts` | 按 test→build→static→security 执行 PRD 中由契约冻结的结构化检查；默认不经 shell，只有契约显式声明时使用指定 shell；调用方正常存活时，超时复用进程树终止单源并等待整棵树退出；门禁/异常轮回写及结构化 Validator pass/fail 的引擎状态转移共享 notes/重试/blocked 规则。父进程异常后的活动进程协调尚未实现 |
...
| workspace 锁 | `src/engine/lock.ts`、`src/engine/fs-atomic.ts` | 0.33.3 当前使用 engine.lock 文件做 O_EXCL/pid 检查、stale 自动接管和轮首重建；重建失败不会停止旧 writer，run/repair 之外仍有不持锁写入口，因此不构成崩溃后隔离。fs-atomic 只保证单文件完整替换（ADR-008/014/021） |
...
workspace 是运行边界：progress 记录学习，evidence 追加普通/TDD 门禁、轮次、篡改、Validator claim、调用凭证、截图等事件；Agent stdout/stderr 实时 tee，异常时只留最近 2000 字符，成功 transcript 不落盘。引擎把绝对项目根和实际 workspace 注入 agent，宿主 hook 只在二者与当前 Git 根配对时使用外部 workspace。`validation-result.json` 只作为单轮瞬时 IPC，调用前清旧、消费后删除，崩溃残留由新 request ID 拒绝。report 从需求、状态与分源证据派生静态报告；关键覆盖写走原子替换。当前 `engine.lock` 只让正常 run/repair 路径单写，不能证明父进程异常后其后代已经退出；手动 report、`prd-to-json` 与 `/review-loop` 写入也未共享同一机械租约。Issue #106 / ADR-021 将统一正式写入口，并在活动状态不明时保留隔离。PRD guard 冻结运行期需求并恢复篡改；自动报告沿用该冻结快照，手动报告才读磁盘。state 缺失可迁移 legacy，存在但损坏则所有展示面统一 fail-closed 为未验证（ADR-007、008、014、015、016、017、021）。
