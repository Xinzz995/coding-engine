...
| 主循环 | `src/engine/loop.ts`、`loop-instructions.ts`、`loop-lifecycle.ts`、`loop-observability.ts` | `loop.ts` 只编排 Developer ⇄ Validator 状态机与门禁顺序；指令渲染、启动/收敛生命周期和调用/篡改证据分别由支持模块单源负责。状态机按 qualityChecks→TDD 门禁→Validator 排序，生成 Validator 精确目标、消费结构化 claim 后写 verdict/签发凭证（ADR-009、013、015、016、017） |
