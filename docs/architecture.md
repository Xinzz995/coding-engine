...
| 知识库体检 | `src/doctor/doctor.ts` | `coding-x doctor` 检查 frontmatter / updated / AGENTS.md 索引 / 相对链接，并核对机械门禁、全局模型目录/PRD 映射与 workspace Git 隔离（ignore + 已跟踪文件）；runDoctor/renderDoctorReport 纯函数，cli 渲染并定退出码 |
