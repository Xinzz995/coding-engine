...
| 模块 | 路径 | 职责 |
|---|---|---|
| CLI 入口 | `src/cli.ts` | 参数解析、启动循环/仪表盘，提供 `models` 模型发现入口 |
...
| 模型发现 | `src/engine/model-discovery.ts` | 检查三 runner 认证；通过 Codex app-server `model/list` 分页列举当前可用模型，Claude/Cursor 无公开机器接口时诚实降级 |
| 模型预检 | `src/engine/model-preflight.ts` | 循环启动前组合 schema、runner、CLI 覆盖、待执行 story 与发现三态；无效即停，不启动 agent/dashboard |
...
模型路由数据流：`prd-to-json` 在 story 定稿后让用户选择 runner 与五个模型，并按仓库事实派生每个 story 的 `difficulty/reason` → 启动时 preflight 严格校验 schema、runner 与当前环境的有效模型 → loop 按 `difficulty + state.escalated + CLI override` 解析本轮实际模型 → agent 返回后引擎恢复 agent 对 `escalated` 的任何改写，并把实际模型、来源、档位和升级触发写入 `evidence.jsonl` → dashboard/status/report 分开展示「配置路由」与「实际命中」，不用前者伪装后者。
