...
| 证据索引 | `src/engine/evidence.ts` | evidence.jsonl 的 schema 单源（iteration/gate-run/tamper/screenshot-claim 四类判别联合）与追加/读取（坏行与未知 type 跳过计数）；失败门禁输出尾部与 Validator 正常打回 notes 共用 2000 字符边界；loop 写机械记录，builder/validator 按指令登记截图，验证报告消费并按 source 区分信任级别 |
...
源 PRD（意图真相）经 prd-to-json 派生为 workspace `prd.json`（执行需求）并初始化 `state.json`；builder 只实现一个 story，机械门禁通过后由 validator 核对，候选 `passes=true` 只有获得引擎 `validated=true` 凭证才算有效通过。异常/no-op/验收不完整轮不会带走未签发的通过态，连续无进展触发 stall 熔断；全部 story 有效通过或 blocked 时分别以 0/3 收敛（ADR-009、013）。

workspace 是运行边界：progress 记录学习，evidence 追加门禁/轮次/篡改/截图等事件，report 从需求、状态与证据派生静态报告；`engine.lock` 让 run/repair 单写，关键覆盖写走原子替换。PRD guard 冻结运行期需求并恢复篡改；自动报告沿用该冻结快照，手动报告才读磁盘。state 缺失可迁移 legacy，存在但损坏则所有展示面统一 fail-closed 为未验证（ADR-007、008、014）。
