...
本仓库当前有 65 份带状态文档，其中 45 份为 `done`；`docs/superpowers/` 约 964 KB，绝大多数是已完成 spec/plan。状态标记解决了“能否辨认历史”，但没有解决两个后续问题：
...
例如 `docs/superpowers/plans/x.md` 移到 `docs/archive/superpowers/plans/x.md`。不再按年份增加一层：文件名与 frontmatter 已含时间，保留原相对树更容易判定目标且避免同名目录规则分叉。
...
本功能自己的 spec/plan 在尚未合并或发布时保持 `active`，不借本次迁移提前归档。`docs/superpowers/specs/` 与 `docs/superpowers/plans/` 各补 README，保证活文档入口在未来清空时仍可被 Git 与 AGENTS 索引保留。
