...
**Spec:** `docs/specs/2026-07-02-docs-knowledge-base-design.md`
...
```
Expected: 无输出（仓库中除 docs/archive/ 历史文档外不再有任何 init-rules / AGENTS-template 引用）。
...
- Produces: 本仓库自己的知识库，提交入库；AGENTS.md 索引表必须收录既有 `docs/specs/` 与 `docs/plans/`。
...
| PRD | `docs/prds/` | prd-generate 产出 |
| 功能设计文档 | `docs/specs/` | 需求设计流程 产出的设计规格 |
| 实施任务计划 | `docs/plans/` | 实施规划流程 产出的分任务计划 |
...
grep -n 'docs/specs\|docs/plans' AGENTS.md
head -6 docs/architecture.md
npm run typecheck && npm test
```
Expected: AGENTS.md ≤100 行；5 个文件存在；索引表含 `docs/specs/`、`docs/plans/` 两行；frontmatter 完整；typecheck/test 通过（确认零代码影响）。
