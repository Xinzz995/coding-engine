...
**Architecture:** 与 `src/status/` 同构的纯函数三段式：`collectReport`（读 workspace → `ReportData`，三态容错）→ `renderReportHtml`（`ReportData` → HTML 字符串，零浏览器 JS）→ `writeReport` 编排落盘；cli 与 loop 两个调用方。截图相对路径引用不内嵌。规格：`docs/specs/2026-07-08-static-html-report-design.md`。
...
**Files:**
- Modify: `docs/specs/2026-07-08-static-html-report-design.md`（status → done）
- Modify: `docs/plans/2026-07-08-static-html-report.md`（status → done）
...
Run: `node dist/cli.js report --workspace .coding-x-local/fixtures/study-report-dogfood/.workspace && ls -la .coding-x-local/fixtures/study-report-dogfood/.workspace/report.html`
Expected: 退出码 0、打印 `📄 验证报告:` 路径、report.html 生成；浏览器打开抽查：61 个工件按 story 归类、review-2026-07-07.md 渲染、红旗区列出 prd.tampered-20260707-165439.json。冒烟后删除生成物：`rm .coding-x-local/fixtures/study-report-dogfood/.workspace/report.html`（fixture 不留生成物）。
...
```bash
git add docs/specs/2026-07-08-static-html-report-design.md docs/plans/2026-07-08-static-html-report.md
