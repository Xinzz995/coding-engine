...
**Spec:** `docs/specs/2026-07-22-global-model-catalog.md`
...
- Update status after implementation: `docs/specs/2026-07-22-global-model-catalog.md`
- Update status after implementation: `docs/plans/2026-07-22-global-model-catalog.md`
...
- Optional local dogfood artifacts: `.coding-x-local/fixtures/study-report-dogfood/`（不纳入发布资产）
...
- 隔离条件：执行前 clone 工作树干净且没有 Git remote；完整脱敏材料另存于本地忽略目录 `.coding-x-local/fixtures/study-report-dogfood/.workspace/archive-run-20260722-global-model-catalog/`，原始临时根为 `/private/tmp/coding-x-dogfood-20260722.4nUyFn`（可能被系统清理，本节仍保留可独立复核的持久摘要）。
