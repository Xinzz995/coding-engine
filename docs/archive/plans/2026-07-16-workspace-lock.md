...
**Architecture:** 新模块 `src/engine/lock.ts`（O_EXCL 原子创建 + pid 活性三分支判定 + 信号/exit 钩子清锁 + verify 自愈）与 `src/engine/fs-atomic.ts`（writeFileAtomicSync）；loop/cli-repair 两接入点，doctor 加 stale 锁建议项。规格：`docs/specs/2026-07-16-workspace-lock-design.md`。
...
**Files:**
- Modify: `docs/specs/2026-07-16-workspace-lock-design.md`（status 置 done）
- Modify: `docs/plans/2026-07-16-workspace-lock.md`（status 置 done）
...
```bash
git add docs/specs/2026-07-16-workspace-lock-design.md docs/plans/2026-07-16-workspace-lock.md
