...
2026-07-17 dogfood 真实跑（`docs/archive/dogfood/2026-07-17-dogfood-findings.md`）实证四个缺陷，共同根因：**引擎对 agent 进程异常结局（超时/中断/空转）一律「当轮作废下轮重试」，但作废不回滚产物、不留痕，重试按 state 前进而非按验收完备性前进**。
