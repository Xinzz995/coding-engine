...
- 2026-08-18 质量契约的 `paths` 是已知改动到必要检查的调度映射，不是安全白名单：范围必须从固定起点到当前 head 的完整 Git 清单机械计算；无路径声明始终执行，任何未被模式覆盖的路径、选择异常或零项都回退本平台全量。PR/合并按范围运行，独立定时 full 用来发现宿主、依赖和工具链漂移；不能拿历史绿色替代当前提交证据（ADR-030）。
- 2026-08-18 被测提交不能把“同一提交通过 Review 后才发生的远端状态、补签结果或耗时”列为提交内容的完成条件：事后补写必然改变 head 并使原证据失效，形成不可满足的循环。提交只固定测试夹具与边界；后置运行结果写入绑定该 head 的 Issue、评论或机器证明，由收口流程核对。

- 2026-08-17 慢外部事实晚到时，不应把已经机械结算的昂贵本地工作当作缓存失效：先冻结并重核本地结果绑定，再只刷新外部事实，最后在同一短租约内补签派生产物。候选 Dogfood 的恢复命令必须再次逐文件证明当前 CLI 身份，并夹取 Story/Review/Runner/Git/PR/契约当前性；任一变化或远端非 ready 都零写入。agent 可写 workspace 中的旧摘要不能代替候选证据，失败刷新也不能覆盖已有证明。
...
- 2026-07-31 含真实进程树、临时 Git 仓库和平台隔离器的 Vitest 全量回归必须按文件串行；并行争抢会让用于识别真实卡死的 5 秒/10 秒短时限产生级联假失败。不能靠全局放宽时限、盲目重试或人工维护易漏的“串行测试名单”掩盖问题；真实集成流程可有贴合平台实测的独立预算，但单项生产安全判定时限保持不变。平台重试必须是已识别、可证明安全且有明确上限的系统瞬态：Windows 目录 rename 只在目标仍不存在并完整复查 source/安全树/commit check 后，对 `EPERM/EACCES` 按 25/50/100ms 重试；Windows 固定只读组合身份快照只在第一次同时满足 `error.code === 'ETIMEDOUT'` 与 `status === null` 时完整重试一次，两次共享 120 秒单调绝对预算。两者都不得扩大到权限、协议、数据或 unknown 错误。只有五个平台都取得重复稳定数据后才能重新评估并行。
- 2026-08-08 普通 Windows Vitest 为控制真实进程测试成本而 alias identity transport，其绿色只能证明上层身份语义，不能证明生产 PowerShell/CIM 的 timeout、重试或诊断。平台控制逻辑要在无 alias 的非 Windows 单测中经仅测试模块可导入的 controlled seam 注入系统命令与假单调时间；production 禁止导入 seam，公开入口不接受 runtime。真实 Windows 结论仍由不加载 alias 的兼容检查与 standard-user native job 给出。重试只接受完整的新快照，不缓存、不拼接部分 stdout、不回退 pid-only；stdout/stderr 各自最多捕获 16 KiB，诊断只可输出固定枚举阶段等受控字段，不得回显原始 stdout/stderr、机器、boot、process identity、完整命令或环境。
...
- 2026-08-05 Agent 提示词授予的写法必须与引擎委托合同逐字相容：append-only 文件只能在末尾新增完整记录，不能同时要求更新顶部汇总段。可复用学习先写入本轮追加记录，项目级归纳由收口流程升格；历史汇总段只读兼容。否则 Agent 无论遵循哪条指令都会被另一条判错，形成确定性失败而非模型偶发失误。
- 2026-08-05 受管进程的系统结算与 workspace 业务改动是否被接受是两条独立事实：后者失败不能抹掉前者已经完成的可信收口，也不能仅凭错误文字推断进程已结束。临时域只消费 operation 内部私有签发的 proof 与 drain reason，并按调用面原有政策清理——Developer 排除残留后代，Review 只接受自然结束；目录身份复核失败仍保留现场，workspace 隔离和原始错误保持不变。
- 2026-08-09 供应商 Runner 的被动事件流可以包含多条模型消息，但数量放宽不能变成结果权威放宽：所有消息都要逐条做能力与危险声明扫描，只有同一次调用由供应商原生最终输出通道给出的结果才进入业务解析。最终输出文件同样属于不可信临时域，必须绑定身份、限制唯一普通文件与大小、在受管进程自然结算后从受信句柄读取；缺失、替换、额外对象或解析失败一律失败关闭。任何事件协议放宽都要同步提升规则和工具策略版本，使旧 Review 机械失效。
- 2026-08-13 认证 Runner（可签发验收凭证的 Validator Runner）版本升级协议：新版本一律先按 unsupported-version 判不可验证，升级 = ①对照 `validator-runner-profile.ts` 的固定 argv、`--disable` 清单与 `HOST_CONTEXT_ISOLATION_EXPECTATIONS` 逐项重审计新版本参数语义（`--help`/config 文档 + 实测）；②同步调整 profile args 与隔离参数期望集；③更新 `CURRENT_RUNNER_VERSIONS` 固定值；④在真实项目跑通「参数事实核对 + canary 反测 + 验证闭环」并留 dogfood 记录。四步未完成前不得临时放宽；Claude/Cursor 补齐原生边界后走同一协议直接升级为认证 Runner，无需重开 ADR-025。真机审计会翻案草稿假设：`code_mode*` 是 Codex 命令执行宿主不可禁用（禁用则 Validator 只能读不能执行）。
- 2026-08-14 逐 Story 验收范围不能由最终提交的父节点推断：引擎必须在 Developer 前冻结 Story 起点，request/result/凭证同时绑定起点、终点和整段变化摘要；旧候选缺起点时保留实现但不可验证，绝不补造 `HEAD^`。精确提交检出还必须按质量契约建立并持续核对冻结的 `origin/<defaultBranch>` 引用，引用名和目标进入环境身份；否则同一项目命令会因开发仓有引用、验证仓无引用而得到不同结局（ADR-026）。
- 2026-08-17 GitHub Ruleset 详情里的 `bypass_actors` 是权限可见字段：缺失不等于空。只读核验若看不到它，只能用最新 history 版本的完整 state 补证，并在读取前后夹取最新版本标识、复读当前详情、逐字段比较规则核心；任一变化或差异都失败关闭。不要为了减少 API 调用把历史结果做跨检查点缓存。
- 2026-08-13 环境重定向式隔离（把 CODEX_HOME/HOME/XDG 从用户真实目录重定向到引擎干净临时域）不能用「在已重定向环境里运行时种 sentinel 看缺席」验证——canary 眼中的「宿主」就是引擎临时域，种 sentinel 是自造真实不存在的污染，且在全盘可读 runner 上还会误测成「主动读能力」。这类隔离的硬保证是静态参数/环境事实（`assertHostContextIsolation` 核对重定向落域 + 临时域除预置认证外为空 + `--ignore-*`/`--disable` 全集），运行时 canary 只保留引擎能机械观察且有区分力的反测（越界写、受控命令、结构化回执、凭据不外泄、进程与域收口）。凡「让模型主动读/搜索 X 期望失败」的检查在全盘可读 runner 上都无区分力，不得作为隔离证明（ADR-025 诚实边界）。
