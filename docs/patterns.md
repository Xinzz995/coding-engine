...
- 2026-07-27 PR 的行为验收与交付后置条件不能混成同一个 Reviewer 任务：Spec Review 只判断改动是否满足意图；机械检查、全部 Review 轴、GitHub 门禁和发布状态由引擎按顺序收口。否则要求一个轴证明“包含自己在内的 Review 已完成”，必然形成循环证明并安全失败。审查包要带绑定当前 head 的前置机械事实，也要明确哪些后置状态由引擎判定。
