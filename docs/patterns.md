...
- 2026-07-22 共享 state 里的 agent 声明与引擎事实必须分层：builder 只能写 `passes=true` 候选，Validator 只能写 `source=validator` 的结构化 claim，验收凭证、retry/blocked/notes 与最终 verdict 全由引擎消费 claim 后写入；所有选 story、收敛、status、dashboard、report 必须复用同一个当前 HEAD/有序 AC 凭证判断。机械检查失败或合法 Validator failed 才清除实现候选；超时、跳过、协议 invalid、旧凭证缺失或错绑只撤销验收结论并保留候选进入 validation-only，仍必须失败关闭且不得进入最终 Review（ADR-013、015、020）。
