...
- 2026-07-31 含真实进程树、临时 Git 仓库和平台隔离器的 Vitest 全量回归必须按文件串行；并行争抢会让用于识别真实卡死的 5 秒/10 秒短时限产生级联假失败。不能用放宽单测时限、重试或人工维护易漏的“串行测试名单”掩盖问题；只有五个平台都取得重复稳定数据后才能重新评估并行。外层 CI 预算应给串行基线保留充足余量，但单项安全判定时限保持不变。
