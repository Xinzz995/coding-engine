...
- 2026-08-13 认证 Runner（可签发验收凭证的 Validator Runner）版本升级协议：新版本一律先按 unsupported-version 判不可验证，升级 = ①对照 `validator-runner-profile.ts` 的固定 argv 与 `--disable` 清单逐项重审计新版本参数语义（`--help`/config 文档 + 实测）；②同步调整 profile args 与 sentinel 种植位置（`validator-canary.ts`，位置集合属审计事实）；③更新 `CURRENT_RUNNER_VERSIONS` 固定值；④在真实项目跑通 canary 反测与验证闭环并留 dogfood 记录。四步未完成前不得临时放宽；Claude/Cursor 补齐原生边界后走同一协议直接升级为认证 Runner，无需重开 ADR-025。
- 2026-08-13 引擎侧 canary 反测的判据只能是引擎可独立观察的事实：sentinel token 在完整输出与回执中的缺席、越界写探针文件不存在、检出探针内容与精确 HEAD 出现、respondToken 结构化回执严格解析、临时域无符号链接；模型自述（READ-DENIED 等锚定词）只作诊断辅助。执行器内部故障（超时、异常退出、输出超限、探针无法建立）不产出证据而不是产出「全 unverifiable」证据——解析器按 canary-missing 失败关闭，负向检查在输出不完整时的「token 缺席」不可信。
