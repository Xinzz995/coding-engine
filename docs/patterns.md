...
- 2026-07-24 provider 的实际输入额度与理论上下文窗口不是同一个概念。超限处理必须在首次调用前按完整 prompt 预算，无损拆分 source、diff 或两者，使叶子片段共同覆盖完整评审空间并逐片 fail closed；不得截断证据、丢弃失败片段或让后一个片段降低前一个片段的严重度。不同 PR 共享 provider 容量时还必须在仓库级串行，单个 PR 内串行不够。历史 GitHub Models 免费层暴露过单次输入、每分钟和每日请求限制；当前 Copilot CLI 同样必须以真实调用结果而不是 catalog 宣传值为准。
- 2026-07-24 隔离评审轴也要隔离来源所有权：Spec 来源只进入 Spec 轴，工程标准进入 Standards 和 Deep；Deep 可复用工程标准检查结构问题，但不能再载入 Spec 后重新裁决产品范围。把两类来源都塞进 Deep 既破坏轴独立，也会让高风险改动因重复上下文稳定耗尽分片预算。
- 2026-07-24 评审来源目录应是可信允许范围，不是“每次全量塞给模型”的清单：Spec 只读取 PR 明确关联及本次直接修改的规格；没有独立规格时作者必须明说 PR 意图就是完整 Spec。路径越界、不在允许范围或当前 head 不存在都 fail closed，不能以模糊相关性静默选择或丢弃来源。
