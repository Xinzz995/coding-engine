...
这里的“只读”描述 Runner 被授予的能力和引擎接受结果的条件，不等于所有平台、所有异常下的
操作系统权限证明。临时域异常保留时，只有 POSIX 已绑定句柄完成对象、内容和权限复核才能记录
`restricted`；其他情况明确返回保护状态不可验证。该状态也不证明收口前无人读取或复制内容，
具体边界以 ADR-018 为准。

### Runner 合同

- Codex：使用独立权限配置显式拒绝文件系统根、系统临时目录和 `/tmp`，只向当次临时审查包
  重新授予读权限，并关闭 Runner 沙箱网络；同时使用临时会话、忽略用户配置和项目执行规则，
  禁用 CLI 支持关闭的外部能力并严格校验结构化事件；
- Codex CLI 若仍把没有受支持关闭开关的内置能力展示给模型，不得把“工具不可见”写成保证。
  结果只有在包外读取、写入、危险副作用和外部能力均被机械反测阻断时才有效；正常 Review
  出现任意非被动工具事件仍返回 `unverifiable`；
