...
4. 文档必须说明：同一绝对候选 CLI 按顺序贯穿 workspace init、doctor --shadow、
   workspace apply-prd --shadow 和最终 shadow run；正式 doctor/apply-prd 应因固定 0.33.3 与
   候选 0.34.0 不一致而失败；shadow 三步健康时都退出 7，且 7 不是通过或可交付结论。
