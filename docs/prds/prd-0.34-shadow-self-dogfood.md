---
title: 'coding-x 0.34.0 候选 Shadow 自托管验证'
status: active
updated: 2026-08-02
scope: root
---

# coding-x 0.34.0 候选 Shadow 自托管验证

## Context

coding-x 0.34.0 候选新增受控的 Shadow workspace 准备链。发布前需要让同一候选包在
coding-engine 自身完成 workspace 初始化、正式模式拒绝、Shadow doctor、Shadow PRD 应用、
Developer、Validator 和最终 Review。该运行只证明候选流程能够完整执行，不把 Shadow 结果
升级为正式交付凭证，也不要求 GitHub 证明本地模型 Review 已发生。

本 PR 是候选验证专用分支，在 npm staging 前保持开放且不合并，避免改变候选绑定的 main
提交。PR #65 是旧候选历史，不属于本任务范围。

## Goals

- 用固定 0.34.0 候选包在 coding-engine 自身执行完整 Shadow 流程。
...
| 原则                | 适用性与设计裁决                                                                                       | 验证证据                                         |
| ------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------ |
| 1. 可证伪完成合同   | 适用。Story 明确限定新增文件、必须出现的边界和全量检查；任一缺失即失败。                               | 文件 diff、Validator 逐项结论、仓库检查。        |
...
- [ ] 新增 `docs/specs/2026-08-02-candidate-shadow-boundary.md`，不修改本 PRD 之外的其他既有文件。
