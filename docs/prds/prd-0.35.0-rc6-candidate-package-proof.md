...
本 PRD 为 RC6 seed 的修订版：首个 seed PR #209 的 Shadow 自托管在最终 Review 中揭示，把一次性
安装目录的绝对路径写成必须逐字匹配的完成合同会破坏可重复验证与失败恢复（deep 轴
RV-DEEP-6efaf3a0c0fe6fdd），而只在实现计划中放宽又会背离规格（spec/engineering 轴
RV-SPEC-1f00513b83f598dc、RV-ENGINEERING-34b54c1bbe3a29c2）。按评审处方，本修订把候选入口身份
改绑「同一 tarball 摘要建立的全新安装 + 入口 CLI SHA-256」，一次性路径降级为现场记录。#209 及其
workspace、requestId、Validator 回执与 Review 结论全部作废，不得复用；候选包本身不受影响。
...
- coding-engine 固定 RC6 CLI：从上述同一 tarball SHA-256 的候选包在仓库外建立的全新独立安装的
...
- 固定 CLI SHA-256：`699b44321519e52827545eb1b62b8fb314a4695913883fe34398a1df080004c4`（每次执行
...
- [ ] 计划逐字包含版本、来源提交、候选运行编号、artifact 名称、ID 与 digest、tarball 大小与
      SHA-256、候选入口建立与重建规则、本轮实际入口的现场记录路径、CLI SHA-256、PR #206 边界，
      并声明十项身份必须同时匹配。
