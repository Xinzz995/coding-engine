...
- [ ] 缺字段、无 frontmatter 跳过、无 docs/ 降级、退出码等行为有自动化测试覆盖
- [ ] Typecheck 通过
...
- [ ] 过期与格式非法均计入退出码 1
- [ ] 过期判定、阈值参数、0 阈值、非 git 降级、非法格式等行为有自动化测试覆盖
- [ ] Typecheck 通过
...
- [ ] 索引缺失路径与文档断链均计入退出码 1
- [ ] 索引路径提取、目录路径、断链、锚点剥离、无 AGENTS.md 降级等行为有自动化测试覆盖
- [ ] Typecheck 通过
...
- [ ] 在本仓库根目录真实运行一次 `coding-x doctor`（`node dist/cli.js doctor` 或等价方式）：命令正常完成、输出四项检查结果、以与检查结果一致的退出码结束，运行结果摘要写入进度说明
- [ ] 全部既有测试与新增测试通过
- [ ] Typecheck 通过
