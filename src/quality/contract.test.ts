...
const CODING_ENGINE_STABLE_REFEREE_VERSION = '0.33.2';
...
describe('readQualityContract', () => {
  ...
  it.skipIf(process.platform === 'win32')('never follows a quality contract symlink', () => {
    const root = mkdtempSync(join(tmpdir(), 'quality-contract-link-'));
    try {
      mkdirSync(join(root, '.coding-x'));
      const outside = join(root, 'outside-quality.json');
      writeFileSync(outside, JSON.stringify(validContract()));
      symlinkSync(outside, join(root, '.coding-x', 'quality.json'));
      expect(readQualityContract(root)).toMatchObject({ status: 'io-error' });
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it.skipIf(process.platform === 'win32')(
    'never follows a quality contract parent directory outside the project',
    () => {
      const root = mkdtempSync(join(tmpdir(), 'quality-contract-parent-link-'));
      const outside = mkdtempSync(join(tmpdir(), 'quality-contract-parent-outside-'));
      try {
        writeFileSync(join(outside, 'quality.json'), JSON.stringify(validContract()));
