...
export interface ReviewPreflightContext {
  ...
  baseSha: string;
  headSha: string;
  pullRequest: Required<Pick<GitHubPullRequestInfo,
...
export type ReviewContextRevalidation =
  | { ok: true }
...
function trackedPaths(root: string, ref: string): string[] {
  return git(root, ['ls-tree', '-r', '--name-only', '-z', ref])
...
function statusPaths(root: string): string[] {
  const entries = git(root, ['status', '--porcelain=v1', '-z', '--untracked-files=all'])
    .split('\0');
  const paths: string[] = [];
...
function allowedDirtyPath(root: string, workspace: string, generated: string[], path: string): boolean {
  ...
  if (workspaceRelative !== '' && !workspaceRelative.startsWith('../')
      && (path === workspaceRelative || path.startsWith(`${workspaceRelative}/`))) return true;
...
export function validatePullRequestIntent(body: string):
  ...
  return {
    ok: true,
    sections: Object.fromEntries(REQUIRED_PR_SECTIONS.map((name) => [name, parsed[name]])) as
      Record<(typeof REQUIRED_PR_SECTIONS)[number], string>,
  };
}

function completePullRequest(pr: GitHubPullRequestInfo): ReviewPreflightContext['pullRequest'] | null {
  if (pr.baseSha === undefined || pr.title === undefined || pr.body === undefined || pr.labels === undefined) {
    return null;
  }
  return {
    number: pr.number,
...
function sourceDocuments(options: {
  ...
  return options.headPaths.filter((path) => (
...
export function revalidateReviewContext(
  ...
  try {
    ...
    if (repository.fullName !== context.baseContract.repository.fullName
    ...
    const currentBase = normalizeText(git(
      context.root,
      ['rev-parse', `refs/remotes/origin/${repository.defaultBranch}`],
    ));
    const currentHead = normalizeText(git(context.root, ['rev-parse', 'HEAD']));
    if (currentBase !== context.baseSha) return { ok: false, message: '评审期间默认分支 base SHA 发生变化' };
    if (currentHead !== context.headSha) return { ok: false, message: '评审期间本地 HEAD 发生变化' };
    ...
    if (current.headSha !== context.headSha || current.baseSha !== context.baseSha
        || current.baseBranch !== context.pullRequest.baseBranch) {
      return { ok: false, message: '评审期间 PR 的 head、base 或目标分支发生变化' };
    }
    if (normalizeText(current.title) !== normalizeText(context.pullRequest.title)
...
export function runReviewPreflight(options: {
  ...
  try {
    ...
    if (!branch) return { status: 'config-error', message: '当前处于 detached HEAD，不能绑定功能分支' };
    if (branch === options.currentContract.repository.defaultBranch) {
      return { status: 'config-error', message: '当前位于默认分支；最终 Review 只允许在功能分支运行' };
    }
    const repository = client.discoverRepository(root);
    if (repository.fullName !== options.currentContract.repository.fullName
        || repository.defaultBranch !== options.currentContract.repository.defaultBranch) {
      return { status: 'config-error', message: '质量契约中的 GitHub 仓库或默认分支与真实远端不一致' };
    }
    try {
    ...
    const baseSha = normalizeText(git(root, ['rev-parse', `refs/remotes/origin/${repository.defaultBranch}`]));
    ...
    if (pullRequest.baseBranch !== repository.defaultBranch) {
      return { status: 'remote-not-ready', message: `PR 目标分支是 ${pullRequest.baseBranch}，不是 ${repository.defaultBranch}` };
    }
    ...
    if (!intent.ok) {
      return { status: 'unverifiable', message: `PR 正文缺少有效内容：${intent.missing.join('、')}` };
    }
    ...
    try { parsedJson = JSON.parse(rawBaseContract); } catch {
    ...
    if (baseParsed.status !== 'ready') {
      return { status: 'unverifiable', message: `默认分支质量契约无效：${baseParsed.errors.join('；')}` };
    }
    const baseContract = baseParsed.contract;
    if (baseContract.repository.fullName !== repository.fullName
        || baseContract.repository.defaultBranch !== repository.defaultBranch) {
      return { status: 'unverifiable', message: '默认分支质量契约指向另一个 GitHub 仓库或默认分支' };
    }
    const dirty = statusPaths(root).filter((path) => !allowedDirtyPath(
      root,
      options.workspace,
    ...
    for (const path of changedFiles) {
      ...
      if ((base === null && existsAt(root, baseSha, path))
    ...
    for (const file of files) {
      if (isSubmodule(root, baseSha, file.path) || isSubmodule(root, headSha, file.path)) {
        return { status: 'unverifiable', message: `子模块指针无法在本地 Review 中核验：${file.path}` };
      }
      if (isLfsPointer(file.base) || isLfsPointer(file.head)) {
        return { status: 'unverifiable', message: `Git LFS 内容未展开，无法完整评审：${file.path}` };
      }
    }
    ...
    const specPaths = sourceDocuments({
      headPaths, changedFiles, referenceText, patterns: specPatterns,
    });
    const acceptancePaths = sourceDocuments({
      headPaths, changedFiles, referenceText, patterns: acceptancePatterns,
    });
    ...
    const message = error instanceof GitHubQualityError
      ? error.message
