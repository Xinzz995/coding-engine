...
describe('assessReviewRisk', () => {
  ...
  it('rebuilds reviewer escalation identically from persisted primary-axis results', () => {
    const initial = assessReviewRisk(context());
    const escalated = applyReviewerRequestedDeepReview(initial, [
      { axis: 'spec', requestDeepReview: true },
      { axis: 'engineering', requestDeepReview: false },
    ]);

    expect(escalated).toMatchObject({
      triggered: true,
      categories: ['reviewer-request'],
      reasons: ['Spec 或工程 Reviewer 主动升级为深度结构评审'],
    });
    expect(
      applyReviewerRequestedDeepReview(initial, [
        { axis: 'spec', requestDeepReview: true },
        { axis: 'engineering', requestDeepReview: false },
        { axis: 'deep', requestDeepReview: false },
      ]),
    ).toEqual(escalated);
    expect(
      applyReviewerRequestedDeepReview(escalated, [{ axis: 'spec', requestDeepReview: true }]),
    ).toEqual(escalated);
  });

  it('does not let a deep-axis response retroactively change the bound risk identity', () => {
    const initial = assessReviewRisk(context());
    expect(
      applyReviewerRequestedDeepReview(initial, [
        { axis: 'spec', requestDeepReview: false },
        { axis: 'engineering', requestDeepReview: false },
        { axis: 'deep', requestDeepReview: true },
      ]),
    ).toEqual(initial);
  });
});
