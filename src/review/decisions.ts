...
export function validateP1DeferralIssue(
  ...
  if (!issue.labels.includes('quality-p1-deferral')) errors.push('延期 Issue 缺少 quality-p1-deferral 标签');
  ...
  if (!owner || !reason || !expiry || !followUp) errors.push('延期 Issue 缺少负责人、原因、到期日或跟进事项');
...
export function unresolvedBlockingFindings(options: {
  ...
  now?: Date;
}): { unresolved: ReviewFinding[]; decisionErrors: string[] } {
  ...
  for (const finding of options.findings) {
    ...
    const candidates = options.decisions.filter((decision) => (
      decision.findingId === finding.id && decision.headSha === options.headSha
    ));
    ...
    if (current.action === 'p1-deferred') {
      ...
      try {
        const issue = options.client.getIssue(options.contract.repository.fullName, current.issue);
        ...
        decisionErrors.push(`${finding.id}：${error instanceof Error ? error.message : String(error)}`);
        unresolved.push(finding);
      }
    }
  }
