...
const IDENTITY_PARENT = new URL('../identity.ts', import.meta.url).href;
const IDENTITY_PRODUCTION_TRANSPORTS = new Set([
  new URL('../windows-identity-transport.js', import.meta.url).href,
  new URL('../windows-identity-transport.ts', import.meta.url).href,
]);
const IDENTITY_TEST_TRANSPORT = new URL('../windows-identity-test-transport.ts', import.meta.url)
  .href;
...
export async function resolve(specifier, context, nextResolve) {
  ...
  } else if (context.parentURL === IDENTITY_PARENT) {
    const resolvedFromParent = new URL(specifier, context.parentURL).href;
    if (IDENTITY_PRODUCTION_TRANSPORTS.has(resolvedFromParent)) {
      return nextResolve(IDENTITY_TEST_TRANSPORT, context);
    }
  }
