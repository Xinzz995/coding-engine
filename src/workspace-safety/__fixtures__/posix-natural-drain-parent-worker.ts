...
const escapedPidPath = process.argv[3];
if (!workspace || !escapedPidPath) throw new Error('workspace and escaped pid paths are required');
  ...
  async (operation) =>
    runDarkPosixSupervisedOperation(operation, {
      ...
      hooks: {
        ...
        onRootResult: () => {
          writeFileSync(process.stdout.fd, `${JSON.stringify({ phase: 'natural-drain' })}\n`);
        },
      },
    }),
);
