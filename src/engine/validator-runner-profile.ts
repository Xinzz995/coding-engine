...
export interface ValidatorRunnerProfileRequest {
  ...
  readonly platform: NodeJS.Platform;
  readonly architecture: string;
  /** null = runner-default 路由：不传 --model，凭证与 canary 以 null 绑定该事实。 */
  readonly model: string | null;
  ...
  /** 结构化验收协议版本；claim 合同由该版本的引擎解析器单源定义（ADR-015）。 */
  readonly claimProtocolVersion: number;
...
export interface ValidatorRunnerTemporaryDomain {
  ...
  /**
   * Fixed claim location inside the only sandbox-writable output area. The claim stays on the
   * ADR-015 file protocol (one-time request, atomic write, strict parse); structured stdout is
   * intentionally not a claim transport so there is a single source of truth.
   */
...
export interface ValidatorRunnerProfile {
  ...
  readonly claimTransport: 'result-file';
  readonly claimProtocolVersion: typeof VALIDATION_PROTOCOL_VERSION;
  readonly commandContractSha256: string;
...
/** 由 identityRoot 推导的确定性临时域布局；引擎创建目录与 profile 渲染共用同一来源。 */
export function validatorRunnerTemporaryDomain(
  root: string,
  runner: AgentKind,
): ValidatorRunnerTemporaryDomain {
  return temporaryDomain(root, runner);
}
...
function temporaryDomain(root: string, runner: AgentKind): ValidatorRunnerTemporaryDomain {
  ...
  return {
    ...
    resultPath: join(temp, VALIDATION_RESULT_FILE),
...
/** Receipt 绑定用的 canary 证据摘要（raw hex）；与 profileDigest 使用同一稳定序列化。 */
export function validatorCanaryEvidenceDigest(evidence: ValidatorRunnerCanaryEvidence): string {
...
function buildProfile(request: ValidatorRunnerProfileRequest): ValidatorRunnerProfile {
  ...
  const model =
    request.model === null ? null : assertBoundedString(request.model, 'Runner model', 300);
  ...
  if (request.claimProtocolVersion !== VALIDATION_PROTOCOL_VERSION) {
    throw new InvalidProfile(
      `Validator claim 协议版本 ${request.claimProtocolVersion} 未经 ` +
        `${VALIDATOR_RUNNER_PROFILE_POLICY_VERSION} 审计（当前审计版本 ${VALIDATION_PROTOCOL_VERSION}）`,
    );
  }
