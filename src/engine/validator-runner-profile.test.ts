...
const CLAIM_SCHEMA = JSON.stringify({
  type: 'object',
  ...
  it('keeps the audited Codex profile unverifiable until an exactly bound real canary exists', () => {
    ...
    expect(profile.args).toEqual(
      expect.arrayContaining([
        ...
        'workspace-write',
        '--output-schema',
        profile.temporary.claimSchemaPath,
        '--json',
        '-',
      ]),
    );
    ...
    expect(profile.environment).toMatchObject({
      PATH: '/usr/bin:/bin',
      HOME: '/private/tmp/coding-x-validator/identity/home',
      CODEX_HOME: '/private/tmp/coding-x-validator/identity/codex',
      XDG_CONFIG_HOME: '/private/tmp/coding-x-validator/identity/config',
      TMPDIR: '/private/tmp/coding-x-validator/identity/tmp',
    });
  ...
  it('rejects case-colliding Windows environment names and creates an isolated Windows identity', () => {
    ...
    expect(resolution).toMatchObject({
      ...
      profile: {
        environment: {
          PATH: 'C:\\Windows',
          USERPROFILE: '/private/tmp/coding-x-validator/identity/home',
          APPDATA: '/private/tmp/coding-x-validator/identity/config',
          LOCALAPPDATA: '/private/tmp/coding-x-validator/identity/data',
        },
      },
    });
  });
