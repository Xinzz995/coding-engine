...
export class ExternalFileLinkSnapshotBudget {
  ...
  constructor(
    ...
    this.#deadline = this.#startedAt + limits.deadlineMs;
  }
  ...
  timeoutError(): ExternalFileLinkSnapshotBudgetError {
    ...
    const workload = this.limits.workload;
    ...
    return new ExternalFileLinkSnapshotBudgetError(
      '外部普通文件链接核对超过统一期限（' +
        `budgetMs=${workload === undefined ? unavailable : this.limits.deadlineMs}, ` +
        `elapsedMs=${elapsed}, ` +
  ...
  completeLinks(count: number): void {
    ...
    const maximum = this.limits.workload?.linkCount ?? this.limits.maxLinks;
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    try {
      ...
      if (
        ...
        if (result.timedOut) throw options.budget.timeoutError();
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    if (
      ...
      const diagnostic = Buffer.concat([result.stdout, result.stderr])
        ...
        .slice(-500);
      throw new ExternalFileLinkSnapshotBudgetError(
        `外部普通文件链接元数据批次 ${index + 1}/${batches.length} 核对失败${
          diagnostic ? `：${diagnostic}` : ''
        }`,
      );
    }
...
export async function snapshotManagedExternalFileLinksWithAdaptiveBudget(
  ...
  const budget = new ExternalFileLinkSnapshotBudget(
    {
      ...limits,
      deadlineMs: externalFileLinkSnapshotBudgetMs(resolved.workload, limits),
